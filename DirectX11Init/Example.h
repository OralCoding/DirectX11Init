...
class Example2 {
public:
  Example2(HWND window, int width, int height, int canvasWidth, int canvasHeight) {
  ...
  auto InitShaders() -> void {
    ...
    if (FAILED(D3DCompileFromFile(L"VS2.hlsl", 0, 0, "main", "vs_5_0", 0, 0, &vertexBlob, &errorBlob))) {
      if (errorBlob) {
        std::cout << "Vertex shader compile error\n" << static_cast<char*>(errorBlob->GetBufferPointer()) << std::endl;
      }
    }

    if (FAILED(D3DCompileFromFile(L"PS2.hlsl", 0, 0, "main", "ps_5_0", 0, 0, &pixelBlob, &errorBlob))) {
      if (errorBlob) {
    ...
    const D3D11_INPUT_ELEMENT_DESC ied[] = {{"POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0}, {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 16, D3D11_INPUT_PER_VERTEX_DATA, 0},};

    device->CreateInputLayout(ied, 2, vertexBlob->GetBufferPointer(), vertexBlob->GetBufferSize(), &layout);
    deviceContext->IASetInputLayout(layout);
  }

  auto Initialize(HWND window, int width, int height, int canvasWidth, int canvasHeight) -> void {
    ...
    if (FAILED(D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags, featureLevelArray, 1, D3D11_SDK_VERSION, &swapChainDesc, &swapChain, &device, NULL, &deviceContext))) {
    ...
    {
      const std::vector<Vertex> vertices = {{{-1.0f, -1.0f, 0.0f, 1.0f}, {0.f, 1.f},}, {{1.0f, -1.0f, 0.0f, 1.0f}, {1.f, 1.f},}, {{1.0f, 1.0f, 0.0f, 1.0f}, {1.f, 0.f},}, {{-1.0f, 1.0f, 0.0f, 1.0f}, {0.f, 0.f},},};

      D3D11_BUFFER_DESC bufferDesc;
      ZeroMemory(&bufferDesc, sizeof(bufferDesc));
      bufferDesc.Usage = D3D11_USAGE_DYNAMIC;                                     // write access access by CPU and GPU
      bufferDesc.ByteWidth = static_cast<UINT>(sizeof(Vertex) * vertices.size()); // size is the VERTEX struct * 3
    ...
    // Create an index buffer
    {
      ...
      D3D11_SUBRESOURCE_DATA indexBufferData = {nullptr};
  ...
  auto Clean() -> void {
    ...
    if (canvasRenderTargetView) {
      ...
      canvasRenderTargetView = nullptr;
    }
    if (colorSampler) {
